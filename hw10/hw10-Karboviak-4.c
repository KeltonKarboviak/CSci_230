...
int main(int argc, char** argv) {
	void *handle;
	uchar* (*LoadFile)(char*, int*);
	int (*SearchSequence)(uchar*, uchar*, int, int, int*, int*);
	int (*DisplayForwardPrimer)(uchar*, int, int, int, int);
	int (*DisplayReversePrimer)(uchar*, int, int, int, int);

	handle = dlopen("./hw10-lib-Karboviak.so", RTLD_LAZY);
	if (!handle) {
		printf("\n%s\n", dlerror());
	...
	LoadFile = dlsym(handle, "LoadFile");  // Load in Dynamic LoadFile()
	SearchSequence = dlsym(handle, "SearchSequence");  // Load in Dynamic SearchSequence()
	DisplayForwardPrimer = dlsym(handle, "DisplayForwardPrimer");  // Load in Dynamic DisplayForwardPrimer()
	DisplayReversePrimer = dlsym(handle, "DisplayReversePrimer");  // Load in Dynamic DisplayReversePrimer()
